...
def table_horizontal(items, full, level):
	if len(items) < 1:
		return
	print indent(level)+'<table width="100%" height="100%">'
	...
	if items[0][1] == '':
		if level < 15:
			...
			table_vertical(sub_items[1:], sub_items[0][2], level+3)
		else:
...
def table_vertical(items, full, level):
	if len(items) < 1:
		return
	print indent(level)+'<table width="100%" height="100%">'
	size = items[0][2]
	...
	if items[0][1] == '':
		if level < 15:
			sub_items = largest(items[0][0])
			table_horizontal(sub_items[1:], sub_items[0][2], level+3)
		else:
			print indent(level+3)+items[0][0]
	else:
		print indent(level+3)+items[0][1]
